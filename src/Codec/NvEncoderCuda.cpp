...
void NvEncoderCuda::AllocateInputBuffers(int32_t numInputBuffers)
{
    ...
    cudaSafeCall(cuCtxPushCurrent(m_cuContext));
    ...
    for (int i = 0; i < numInputBuffers; i++)
    {
        ...
        cudaSafeCall(cuMemAllocPitch((CUdeviceptr*)&pDeviceFrame,
            &m_cudaPitch,
    ...
    CUstream stream)
{
    ...
    cudaSafeCall(cuCtxPushCurrent(device));
    ...
    {
        cudaSafeCall(cuMemcpy2DUnaligned(&m));
    }
    else
    {
        cudaSafeCall(stream == NULL ? cuMemcpy2D(&m) : cuMemcpy2DAsync(&m, stream));
    }
